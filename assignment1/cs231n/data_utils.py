...
def load_CIFAR10(ROOT):
    ...
    ys = []
    for b in range(1, 6):
        f = os.path.join(ROOT, "data_batch_%d" % (b,))  # data_batch_1, data_batch_2, ...
        X, Y = load_CIFAR_batch(f)  # X: (10000, 32, 32, 3), Y: (10000,)
        xs.append(X)
        ys.append(Y)
    Xtr = np.concatenate(xs)
    Ytr = np.concatenate(ys)
    del X, Y
    Xte, Yte = load_CIFAR_batch(os.path.join(ROOT, "test_batch"))  # test_batch
