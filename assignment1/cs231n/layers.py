...
# x表示representation = feature * weight
def svm_loss(x, y):
    """
    ...
    - dx: Gradient of the loss with respect to x  计算x的梯度
    """
    loss, dx = None, None
    ...
    # 计算loss
    scores = x  # [N, C]
    correct_class_scores = scores[np.arange(num_train), y].reshape(-1, 1)  # (N, )
    margins = scores - correct_class_scores + delta  # [N, C]
    margins[np.arange(num_train), y] = 0
    margins = np.maximum(0, margins)
    loss = np.sum(margins) / num_train

    # 计算x的梯度 (这里的x代表linear_svm.py中的scores)
    dx = np.zeros_like(x)
    dx[margins > 0] = 1  # 将大于0的margin置为1，用作指示性函数.
    positive_num_classes = np.sum(margins > 0, axis=1)  # 计算每个样本有多少个margin大于0. [N, 1]
    dx[np.arange(num_train), y] -= positive_num_classes  # 将真实类别的梯度置为负值的样本数 (而其他类别设为+1, 不用管)
    dx /= num_train  # 求平均值
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
